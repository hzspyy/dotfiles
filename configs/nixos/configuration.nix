...
in
{
        ...
        (pkgs.llama-cpp.override {
          ...
          (oldAttrs: rec {
            ...
            src = pkgs.fetchFromGitHub {
              ...
              repo = "llama.cpp";
              tag = "b${version}";
              hash = "sha256-a8wJH97jxxZ2bE5Hk2UDyir2J57JwqFxHOkUTGumz5k=";
              leaveDotGit = true;
              postFetch = ''
  ...
  services.restic.backups.truenas = {
    ...
    exclude = [ "/home/*/.cache" "/home/*/Downloads" "*.tmp" "node_modules" ];
    ...
    pruneOpts = [ "--keep-hourly 24" "--keep-daily 7" "--keep-weekly 4" "--keep-monthly 12" ];
    initialize = true;
  };
  # --- Other Services ---
